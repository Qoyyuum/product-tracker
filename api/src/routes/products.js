/**
 * Product routes - registration, tracking, and verification
 */

import { validateAuth, optionalAuth } from '../middleware/auth.js';
import { generateUUID, calculateHash } from '../blockchain/crypto.js';
import { createBlock, verifyChain, getLatestHash } from '../blockchain/chain.js';
import { corsHeaders } from '../middleware/cors.js';
import { APIError, validateRequired } from '../utils/errors.js';

export async function handleProductRoutes(request, env, action) {
  try {
    switch (action) {
      case 'register':
...
async function addProcessStage(request, env) {
  ...
  // Get latest stage to get previous hash
...
async function getProductByQR(request, env) {
  ...
  return new Response(JSON.stringify({
    ...
    certifications: certifications.results.map(c => ({
      type: c.cert_type,
...
async function verifyProductChain(request, env) {
  ...
  const getPublicKey = async (orgId) => {
